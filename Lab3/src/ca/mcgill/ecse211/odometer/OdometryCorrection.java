...
public class OdometryCorrection implements Runnable {
	...
	/***
	 * This class implements odometry correction of Lab3 on the
	 * 
	 * 
	 * @authorAbedAtassi
	 * @authorHyunSuAn
	...
	public void run() {
		...
		while (true) {
			...
			if (intensity < 0.21) { // Checks if a black line is detected
				...
				if (angle >= 0.00 && angle <= 40) { // Keep increasing y-position of the robot until before the 90
													// degree turn
					odoData.setY(yCount * size - offset); // keep updating Y depending on how many tiles are being
															// traversed
					yCount++; // Increase tile count
					LCD.drawString("case 1 " + yCount + " " + intensity, 0, 5); // draw the case number and number of
																				// tiles traversed
				} else if (angle >= 75 && angle <= 105) { // Keep increasing x-position until 90 degree turn
					odoData.setX(xCount * size - offset); // keep updating X depending on how many tiles are being
															// traversed
					xCount++;
