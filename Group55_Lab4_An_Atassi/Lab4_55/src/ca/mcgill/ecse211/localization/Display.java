...
public class Display implements Runnable {
	...
	public void run() {
		...
		do {
			...
			// Retrieve x, y and theta values
			position = odo.getXYT();

			// Print x,y, and theta information
			DecimalFormat numberFormat = new DecimalFormat("######0.00");
			lcd.drawString("X: " + numberFormat.format(position[0]), 0, 0); // Display the X value
			lcd.drawString("Y: " + numberFormat.format(position[1]), 0, 1); // Display the Y value
			lcd.drawString("T: " + numberFormat.format(position[2]), 0, 2); // Display theta
